...
function buildDiscordPayload(order) {
  ...
  // ── Note ──────────────────────────────────────────────────────────────────
  // Optional free-text from the customer. Trimmed first; only rendered when
  // there is actual content so empty/null/whitespace notes never show a row.
  const noteValue = typeof order.note === 'string' ? order.note.trim() : '';
  ...
  // Append the note as its own full-width field when present.
  if (noteValue) {
    fields.push({
      name:   '📝 Ghi chú',
      value:  escapeMarkdown(noteValue),
      inline: false,
    });
  }
